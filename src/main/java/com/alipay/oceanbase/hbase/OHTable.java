...
public class OHTable implements HTableInterface {
    ...
    public void batch(final List<? extends Row> actions, final Object[] results) throws IOException {
        try {
            ...
            obTableClient.setRuntimeBatchExecutor(null);
        }
    }
    ...
    private void innerDelete(Delete delete) throws IOException {
        ...
        try {
            ...
            throw new IOException("delete table " + tableNameString + " error codes ", e);
        }
    }
                ...
                if (!(row instanceof Put) && !(row instanceof Delete)) {
                    ...
                    if (delete.isEmpty()) {
                        ...
                        for (Map.Entry<byte[], List<KeyValue>> entry : delete.getFamilyMap()
                            ...
                            for (KeyValue kv : keyValueList) {
                                com.alipay.oceanbase.rpc.mutation.Mutation mutation;
                                if(isTableGroup){
                                    KeyValue new_kv = modifyQualifier(kv,(Bytes.toString(family) + "." + Bytes.toString(kv.getQualifier()))
                                            .getBytes());
                                    mutation = buildMutation(new_kv, DEL, isTableGroup);
                                } else {
                                    mutation = buildMutation(kv, DEL, isTableGroup);
                                }
                                if (KeyValue.Type.codeToType(kv.getTypeByte()) == KeyValue.Type.DeleteFamily){
                                    mutations.clear();
                            ...
                            singleOpResultNum += mutations.size();
                            batch.addOperation(mutations);
                        }
                    }
                }
